...
### Oxygen
O20_exp = np.arange(0.1,8.1,0.2)[::4]   # uM O2
...
### model parameters
dil = np.arange(0.01,0.11,0.01)  # dilution rate (1/day)
...
# Nutrients
fin_O2 = np.ones((len(dil), len(O20_exp))) * np.nan
fin_Sd = np.ones((len(dil), len(O20_exp))) * np.nan
fin_Sp = np.ones((len(dil), len(O20_exp))) * np.nan
fin_NO3 = np.ones((len(dil), len(O20_exp))) * np.nan
fin_NO2 = np.ones((len(dil), len(O20_exp))) * np.nan
fin_NH4 = np.ones((len(dil), len(O20_exp))) * np.nan
fin_N2 = np.ones((len(dil), len(O20_exp))) * np.nan

# Biomasses
fin_bHet = np.ones((len(dil), len(O20_exp))) * np.nan
fin_bFac = np.ones((len(dil), len(O20_exp))) * np.nan
fin_b1Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_b2Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_b3Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_bAOO = np.ones((len(dil), len(O20_exp))) * np.nan
fin_bNOO = np.ones((len(dil), len(O20_exp))) * np.nan
fin_bAOX = np.ones((len(dil), len(O20_exp))) * np.nan

# Growth rates
fin_uHet = np.ones((len(dil), len(O20_exp))) * np.nan
fin_uFac = np.ones((len(dil), len(O20_exp))) * np.nan
fin_u1Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_u2Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_u3Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_uAOO = np.ones((len(dil), len(O20_exp))) * np.nan
fin_uNOO = np.ones((len(dil), len(O20_exp))) * np.nan
fin_uAOX = np.ones((len(dil), len(O20_exp))) * np.nan

# track facultative average respiration
fin_facaer = np.ones((len(dil), len(O20_exp))) * np.nan

# Rates
fin_rHet = np.ones((len(dil), len(O20_exp))) * np.nan
fin_rHetAer = np.ones((len(dil), len(O20_exp))) * np.nan
fin_rO2C = np.ones((len(dil), len(O20_exp))) * np.nan
fin_r1Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_r2Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_r3Den = np.ones((len(dil), len(O20_exp))) * np.nan
fin_rAOO = np.ones((len(dil), len(O20_exp))) * np.nan
fin_rNOO = np.ones((len(dil), len(O20_exp))) * np.nan
fin_rAOX = np.ones((len(dil), len(O20_exp))) * np.nan
...
ax1.set_ylabel('Loss rate (per day)', fontsize=fslab)
...
p1 = ax1.contourf(O20_exp, dil, fin_O2, levels=np.arange(0,10.1,0.5), extend='max', cmap=colmap)
p2 = ax2.contourf(O20_exp, dil, fin_NO3, levels=np.arange(20,30.1,0.5), extend='both', cmap=colmap)
p3 = ax3.contourf(O20_exp, dil, fin_NO2, levels=np.arange(0,5.1,0.25), extend='max', cmap=colmap)
p4 = ax4.contourf(O20_exp, dil, fin_NH4, levels=np.arange(0,0.51,0.05), extend='max', cmap=colmap)
p5 = ax5.contourf(O20_exp, dil, (fin_rAOX * nh4_n2_AOX + fin_r2Den*0.5)*1e3, levels=np.arange(0,101,5), extend='max', cmap=colmap)
p6 = ax6.contourf(O20_exp, dil, fin_rAOX * nh4_n2_AOX / (fin_rAOX * nh4_n2_AOX + fin_r2Den*0.5) * 100, levels=np.arange(0,101,5), cmap=colmap)
p7 = ax7.contourf(O20_exp, dil, ((fin_rHet - fin_rHetAer)/fin_rHet) * 100, levels=np.arange(0,101,5), cmap=colmap)
p8 = ax8.contourf(O20_exp, dil, fin_rNOO*1e3, levels=np.arange(0,51,5), extend='max', cmap=colmap)
p9 = ax9.contourf(O20_exp, dil, fin_rAOO*1e3, levels=np.arange(0,51,5), extend='max', cmap=colmap)

# delineate import thresholds
col = 'firebrick'; lw = 1.0
ax1.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax2.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax3.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax4.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax5.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax6.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax7.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax8.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
ax9.contour(O20_exp, dil, fin_O2*1e3, levels=[100], colors='k', linewidths=lw)
...
c1 = ax1.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c2 = ax2.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c3 = ax3.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c4 = ax4.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c5 = ax5.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c6 = ax6.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c7 = ax7.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c8 = ax8.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
c9 = ax9.contour(O20_exp, dil, babbin_conditions, levels=[0.5], colors=col, linewidths=lw)
...
ax1.set_ylabel('Loss rate (per day)', fontsize=fslab)
ax4.set_ylabel('Loss rate (per day)', fontsize=fslab)
ax7.set_ylabel('Loss rate (per day)', fontsize=fslab)
